 ...
 */
 Strategy.prototype.authenticate = async function (req, options) {
  ...
  if(req.query && req.query.bearer) {
     let response;
      try {
         response = await superagent.post('https://app.deepint.net/api/oauth/login').send({
      ...
      if(response && response.body && response.body.result == 'success') {
         ...
         try {
             userInfoRequest = await superagent.get('https://app.deepint.net/api/v1/who').set('x-auth-token', response.body.auth_token);
             userProfileRequest = await superagent.get('https://app.deepint.net/api/v1/profile').set('x-auth-token', response.body.auth_token);
          } catch (error) {
