...
function Strategy(options,verify) {
 ...
 this.name = 'deepint';
 this._authorizationurl = "https://app.deepint.net/oauth/authorize?client_id="+options.clientID;
 this._tokenurl = "https://app.deepint.net/api/oauth/login";
...
Strategy.prototype.authenticate = async function (req, options) {
 ...
 if(req.query && req.query.bearer) {
     try {
         const response = await superagent.post(encodeURI(this._tokenurl)).send({
     ...
     if(response && response.body && response.body.result == 'success') {
         try {
             const userInfoRequest = await superagent.get('https://app.deepint.net/api/v1/who').set('x-auth-token', response.body.auth_token);
             const userProfileRequest = await superagent.get('https://app.deepint.net/api/v1/profile').set('x-auth-token', response.body.auth_token);
         } catch (error) {
